...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-sky-200/80 dark:border-[#132238] bg-white/90 dark:bg-[#040812]/90 backdrop-blur-xl transition-colors duration-200 shadow-2xs">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-2.5 sm:px-6">
        ...
        >
          ...
          <div>
            <span className="text-base font-bold tracking-tight text-slate-900 dark:text-sky-50 leading-none block">
              BugPulse
            </span>
            <p className="hidden md:block text-[11px] text-slate-400 dark:text-[#42698f] mt-0.5">
        ...
        {/* View Switcher Tabs */}
        <nav className="flex items-center rounded-xl bg-slate-100/80 dark:bg-[#091120] p-1 border border-sky-200/80 dark:border-[#132238]">
          <button
            onClick={() => onViewChange('analytics')}
            className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all cursor-pointer ${
              currentView === 'analytics'
                ? 'bg-white dark:bg-cyan-600 text-cyan-700 dark:text-white shadow-2xs font-semibold border border-sky-200/80 dark:border-transparent'
                : 'text-slate-600 dark:text-[#8ec8f2] hover:text-slate-900 dark:hover:text-sky-50 hover:bg-white/60 dark:hover:bg-[#0e1a2f]'
            }`}
          >
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="relative hidden md:block">
            <Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400 dark:text-[#42698f]" />
            <input
              type="text"
              placeholder="Search issues..."
              value={searchQuery}
              onChange={(e) => onSearchChange(e.target.value)}
              className="w-36 lg:w-48 rounded-lg bg-white dark:bg-[#091120] py-1.5 pl-8 pr-3 text-xs text-slate-900 dark:text-sky-50 placeholder-slate-400 dark:placeholder-[#42698f] border border-sky-200 dark:border-[#132238] focus:border-cyan-500 focus:outline-none transition-all"
            />
          </div>
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg border border-sky-200 dark:border-[#132238] bg-white dark:bg-[#091120] text-slate-600 dark:text-sky-300 hover:bg-slate-100 dark:hover:bg-[#0e1a2f] hover:text-slate-900 dark:hover:text-sky-50 transition-all cursor-pointer"
          >
            {theme === 'dark' ? (
          ...
          {isEditor && (
            <button
              ...
              className="flex items-center gap-1 rounded-lg border border-sky-200 dark:border-[#132238] bg-white dark:bg-[#091120] px-2 py-1.5 text-xs font-medium text-slate-700 dark:text-sky-300 hover:bg-slate-100 dark:hover:bg-[#0e1a2f] transition-all cursor-pointer"
            >
          ...
          <button
            ...
            className={`flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-medium border transition-all cursor-pointer ${
              isEditor
                ...
                : 'bg-white dark:bg-[#091120] border-sky-200 dark:border-[#132238] text-slate-500 dark:text-[#8ec8f2] hover:text-slate-800 dark:hover:text-sky-50'
            }`}
          >
            {isEditor ? (
              <>
